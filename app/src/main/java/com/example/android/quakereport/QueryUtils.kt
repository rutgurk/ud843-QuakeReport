...
object QueryUtils {
    ...
    fun extractEarthquakes(): ArrayList<EarthQuake> {
        ...
        try {
            val earthQuakeFeatureCollection = JSONObject(SAMPLE_JSON_RESPONSE)
            ...
            while (counter <  earthQuakeFeatures.length()) {
                val earthQuakeProperties = earthQuakeFeatures.getJSONObject(counter).getJSONObject("properties")
                        earthquakes.add(EarthQuake(earthQuakeProperties.getDouble("mag"), earthQuakeProperties.getString("place"), earthQuakeProperties.getLong("time")))
                counter ++
            }
